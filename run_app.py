...
def main():
    ...
    try:
        ...
        if not db_config:
            ...
            if dialog.exec():
                config_manager.save_config(
                    dialog.host_input.text(), dialog.port_input.text(),
                    dialog.db_input.text(), dialog.user_input.text(), dialog.pass_input.text()
                )
                db_config = config_manager.load_config()
            else:
                sys.exit(0)
        ...
        connector = DatabaseConnector(
            ...
            user=db_config['user'], password=db_config['password'], ssl_mode="require"
        )
        ...
        def open_main_window(user_info):
            ...
            try:
                ...
                # Эта ошибка всё равно попадёт в global_exception_handler, 
                # но мы можем показать пользователю понятное сообщение
