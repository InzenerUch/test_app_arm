...
class SocialDataTab(QWidget):
    ...
    def init_ui(self):
        ...
        for i, (key, label) in enumerate([('civilian','Гражданская одежда'), ('military_headgear','Форма с головным убором'), ('military_no_headgear','Форма без головного убора'), ('distinctive_marks','Отличительные приметы')]):
            ...
            lbl = QLabel("Нет фото"); lbl.setFixedSize(180, 240); lbl.setStyleSheet("QLabel { border: 2px dashed
        ...
        if pd and (isinstance(pd, bytes) or hasattr(pd, 'data')):
            try:
                ...
                if not p.isNull():
                    ...
                    label_widget.setStyleSheet("QLabel { border: 2px solid
                else:
            ...
            label_widget.setStyleSheet("QLabel { border: 2px dashed
    def load_photo(self, photo_type):
        ...
        if path:
            try:
                ...
                getattr(self, f'photo_{photo_type}_label').setStyleSheet("QLabel { border: 2px solid
